...
if [[  ! -e ~/.cache/weather.json || ${weather_code} == "null" ]]; then
	cp -r ~/.config/conky/GreatPanel/icons/${theme}/unknown.png ~/.cache/weather-icon.png
	else
	cp -r ~/.config/conky/GreatPanel/icons/${theme}/${weather_code}.png ~/.cache/weather-icon.png
fi
exit
